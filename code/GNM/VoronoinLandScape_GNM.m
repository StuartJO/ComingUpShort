function [maxKS,KS,P,b,DegCorr,Input] = VoronoinLandScape_GNM(A,A_dist,PDMs,m,Input)
...
for ilvl = 1:nlvl
    ...
    if ilvl==1
        ...
        ind = 1:ndraw*(ilvl - 1);
        if nParams == 1
            fcn_voronoi_select_sptl(P(ind,1),maxKS(ind),ndraw,bounds,pow);
        elseif nParams <= 4
            ptsnew = fcn_voronoi_selectn(P(ind,:),maxKS(ind),ndraw,bounds,pow);
        else
            ptsnew = fcn_custom_select(P(ind,:),maxKS(ind),ndraw,pow);
        end
    end
    ...
    if useParfor
        ...
        parfor i = 1:ndraw          
                [B, btemp{i}] = GNM(PDMs,ptsnew(i,:),m,Input); 
                [maxKSpar(i),KSpar(i,:)] = calc_maxKS(A_vals,A_dist,B);
                Cpar(i) = corr(sum(B,2),ADeg,'Type','Spearman');
        end
        ...
        for i = 1:ndraw
            indHere = indnew(i);
            [B, b{indHere}] = GNM(PDMs,ptsnew(i,:),m,Input) ;
            [maxKS(indHere),KS(indHere,:)] = calc_maxKS(A_vals,A_dist,B);
            DegCorr(indHere) = corr(sum(B,2),ADeg,'Type','Spearman');
        end
