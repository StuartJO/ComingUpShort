...
% delete(['./figures/Plot1/',FitType,'/S1_',form_name,'.png'])
% delete(['./figures/Plot1/',FitType,'/S2_',form_name,'.png'])
% delete(['./figures/Plot1/',FitType,'/S3_',form_name,'.png'])
% delete(['./figures/Plot1/',FitType,'/DistThr_',form_name,'.png'])
...
print(['./figures/Plot1/',FitType,'/PDF_',form_name,'.png'],'-dpng')
clf
...
print(['./figures/Plot1/',FitType,'/CDF_',form_name,'.png'],'-dpng')
...
data = cell(NMdls*4,1);
for i = 1:NMdls
data{i} = squeeze(r0(i,:,1));
data{i+NMdls} = squeeze(r0(i,:,2));
data{i+(NMdls*2)} = squeeze(r0(i,:,3));
data{i+(NMdls*3)} = squeeze(r0(i,:,4));
end
jittercamp = repmat(cmap,4,1);
JitterPlot(data,jittercamp,1)
xticks([(1+NMdls)/2 ((NMdls+1)+(NMdls*2))/2 ((NMdls*2+1)+(NMdls*3))/2 ((NMdls*3+1)+(NMdls*4))/2])
xticklabels({'All connections','Short-range (<30mm)','Mid-range (30-90mm)','Long-range (>90mm)'})
clear ss
for i = 1:NMdls
    ss(i) = scatter(-1,-1,50,cmap(i,:),'filled');
end
...
ylabel({'Proportion of empirical','non-connections captured'})
set(gca,'FontSize',20)
ax = gca;
%AddLetters2Plots(gcf, {FigLbl2},'HShift', -0.1, 'VShift', -0.1,'FontSize',36)
%AddLetters2Plots(gcf, {FigLbl2},'HShift',-.1, 'VShift', -.05,'FontSize',36)
...
print(['./figures/Plot1/',FitType,'/DistThrR0_',form_name,'.png'],'-dpng',['-r',num2str(SaveRes)])
