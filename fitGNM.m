...
if Input.useParfor
    ...
    parfor k = 1:nGenFromBest
        [B,optim_b{k},MdlWeight] = GenMdlVaryParam(PDMs,P_optim,m,Input);
...
if Input.useParfor
    ...
    parfor k = 1:100
        [B,bestDegCorr_b{k},MdlWeight] = GenMdlVaryParam(PDMs,bestDegCorr_P,m,Input);
...
MdlOutput.optim_meanProb = mean(optim_meanProb);
MdlOutput.optim_medianProb = mean(optim_medianProb);
...
MdlOutput.bestDegCorr_meanProb = mean(bestDegCorr_meanProb);
MdlOutput.bestDegCorr_medianProb = mean(bestDegCorr_medianProb);

MdlOutput.optim_EdgeOverlap = optim_EdgeOverlap;
MdlOutput.optim_TND = optim_TND;
MdlOutput.optim_maxRMSE = optim_maxRMSE;
MdlOutput.optim_rd = optim_rd;
MdlOutput.optim_TFdiff = optim_TFdiff;
MdlOutput.optim_TFdiffnc = optim_TFdiffnc;

MdlOutput.bestDegCorr_EdgeOverlap = bestDegCorr_EdgeOverlap;
MdlOutput.bestDegCorr_TND = bestDegCorr_TND;
MdlOutput.bestDegCorr_maxRMSE = bestDegCorr_maxRMSE;
MdlOutput.bestDegCorr_rd = bestDegCorr_rd;
MdlOutput.bestDegCorr_TFdiff = bestDegCorr_TFdiff;
MdlOutput.bestDegCorr_TFdiffnc = bestDegCorr_TFdiffnc;
